...
const MyRequest = () => {
	...
	const filteredRequests = useMemo(() => {
		let filtered = requests.filter((request) => {
			...
			const matchesStatus = filters.status ? request.requestStatus === filters.status : true;

			return matchesSearchTerm && matchesFormType && matchesDateRange && matchesStatus;
		});
					...
					<div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-4">
						...
						<div className="relative col-span-1">
							...
							<select
								...
								onChange={(e) =>
									setFilters((prev) => ({ ...prev, formType: e.target.value, page: 1 }))
								}>
				...
				<div className="flex-grow overflow-auto">
					<table className="w-full table-auto">
						<thead className="sticky top-0 bg-gray-100 border-b z-10">
							<tr>
								{['Name', 'Form Type', 'Date of Submission', 'Status', 'Actions'].map(
									(header) => (
										<th
											key={header}
